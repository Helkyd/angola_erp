...
frappe.ui.form.on('Cadastro de Processo', {
	onload: function(frm) {
		...
		frm.fields_dict.nome_do_advogado.get_query = function(doc){
			return{
				filters:{
					"designation":['like', 'advogad%']
	...
	validate: function(frm){
		...
		if (isNaN(cur_frm.doc.numero_de_processo)){
			if (cur_frm.doc.numero_de_processo.length != 17){
...
frappe.ui.form.on('Cadastro de Processo','numero_de_processo',function(frm,cdt,cdn){
		...
		if (isNaN(cur_frm.doc.numero_de_processo)){
			if (cur_frm.doc.numero_de_processo.length != 17){
				...
				frappe.model.set_value(cdt,cdn,'numero_de_processo',01)
				alert('Numero de Processo tem que ter somente digitos')
...
frappe.ui.form.on('Cadastro de Processo','nome_do_autor',function(frm,cdt,cdn){
	...
	if (cur_frm.doc.numero_de_processo == '0000'){
		alert('Numero de Processo nao pode ser 0000')
		frappe.model.set_value(cdt,cdn,'numero_de_processo',1)
