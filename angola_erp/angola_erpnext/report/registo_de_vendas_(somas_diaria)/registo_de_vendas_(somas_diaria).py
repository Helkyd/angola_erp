...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	for inv in invoice_list:
		...
		row = [
			inv.Ano, mes2_ , inv.Dia, inv.Total
		]
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql(""" select year(posting_date) as Ano, month(posting_date) as Mes, day(posting_date) as Dia, sum(base_grand_total) as Total from `tabSales Invoice` where docstatus =1 and outstanding_amount = 0 %s group by day(posting_date) order by year(posting_date), month(posting_date), day(posting_date)""".format(additional_query_columns or '') %
