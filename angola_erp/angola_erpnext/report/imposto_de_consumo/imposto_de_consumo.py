...
def get_columns(invoice_list, additional_table_columns):
	...
	columns = columns + [_("Total") + ":Currency/currency:120"] + [_("Imp. Consumo") + ":Currency/currency:120"]
...
#	FacturasPOS = frappe.db.sql(""" select year(posting_date) as Ano, month(posting_date) as Mes, sum(paid_amount) as Total, sum(paid_amount*1/100) as Selo from `tabSales Invoice` where is_pos = 1 and docstatus=1 and paid_amount <> 0 %s group by month(posting_date) order by year(posting_date), month(posting_date)""".format(additional_query_columns or '') % conditions, filters, as_dict=1)	
	...
	#Retencoes para ver a % do IPC
	ipc_perct = frappe.model.frappe.get_all('Retencoes',filters={'name':['like', 'IPC']},fields=['percentagem','isencao'])

	if not ipc_perct:
		#procura por Producao
		ipc_perct = frappe.model.frappe.get_all('Retencoes',filters={'name':['like', '%Produ%']},fields=['percentagem','isencao'])

	print('FILTROS')
	print(filters)
	print(type(filters))
	print(ipc_perct[0].percentagem)
	if ipc_perct:
		filters['ipc_perct'] = ipc_perct[0].percentagem
		...
		Facturas1 = frappe.db.sql(""" select year(x.posting_date) as Ano, month(x.posting_date) as Mes, sum(x.paid_amount) as Total, sum(x.paid_amount * %(ipc_perct)s /100) as Selo from (select posting_date, paid_amount from `tabPayment Entry` where payment_type='receive' and docstatus=1 and paid_amount <> 0 and company=%(company)s and posting_date >= %(from_date)s and posting_date <= %(to_date)s UNION ALL select posting_date, paid_amount from `tabSales Invoice` where is_pos = 1 and docstatus=1 and paid_amount <> 0 and company=%(company)s and posting_date >= %(from_date)s and posting_date <= %(to_date)s ) as x GROUP BY Mes ORDER BY Ano, Mes""", filters, as_dict=1)	
