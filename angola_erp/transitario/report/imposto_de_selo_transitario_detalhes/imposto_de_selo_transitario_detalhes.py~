...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql(""" select year(posting_date) as Ano, month(posting_date) as Mes, day(posting_date) as Dia, name, base_grand_total as Total, (base_grand_total*1/100) as Selo from `tabSales Invoice` where name IN (select sii.parent from `tabItem` as i INNER JOIN `tabSales Invoice Item` as sii ON  sii.item_name = i.name where imposto_de_selo) and docstatus =1 and outstanding_amount = 0 %s order by year(posting_date), month(posting_date)""".format(additional_query_columns or '') %conditions, filters, as_dict=1)
