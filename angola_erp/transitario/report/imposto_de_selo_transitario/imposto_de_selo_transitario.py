...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql(""" select year(posting_date) as Ano, month(posting_date) as Mes, sum(base_grand_total) as Total, sum(total_imposto_selo_trans) as Selo from `tabSales Invoice` where name IN (select sii.parent from `tabItem` as i INNER JOIN `tabSales Invoice Item` as sii ON  sii.item_name = i.name where imposto_de_selo) and docstatus =1 and outstanding_amount = 0 %s group by month(posting_date) order by year(posting_date), month(posting_date)""".format(additional_query_columns or '') %conditions, filters, as_dict=1)
