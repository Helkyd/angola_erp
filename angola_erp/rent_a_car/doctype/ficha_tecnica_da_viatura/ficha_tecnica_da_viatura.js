		...
		if (tarifarios){
			...
			*/
			...
			if (cur_frm.doc.entrada_ou_saida_viatura == 'Saida'){
				...
				cur_frm.doc.total_dias = tarifarios.preco_por_dia * (frappe.datetime.get_day_diff(frappe.datetime.now(),cur_frm.doc.data_saida_estacao) + 1) //(moment(frappe.datetime.now_datetime()).format('D') - moment(cur_frm.doc.data_saida_estacao).format('D'))
