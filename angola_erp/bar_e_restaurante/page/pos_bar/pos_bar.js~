...
angola_erp.pos_bar.PointOfSale = erpnext.taxes_and_totals.extend({
	...
	beforeunload: function(e){
		if(this.connection_status == false && frappe.get_route()[0] == "pos-bar"){
			e = e || window.event;
	...
	get_data_from_server: function(callback){
		...
		frappe.call({
			...
			method: "angola_erp.util.pos.get_pos_data",
			freeze: true,
			freeze_message: __("Master data syncing, it might take some time"),
			callback: function(r){
				me.init_master_data(r)
				localStorage.setItem('doc', JSON.stringify(r.message.doc));
				localStorage.setItem('atendbar', JSON.stringify(r.message.atendbar));
...
//				alert("Ainda por ver")
	...
	remove_zero_qty_item: function(){
		...
		this.frm.atendbar["produtos"] = this.produtos;
	},
