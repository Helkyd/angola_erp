...
angola_erp.pos_bar.PointOfSale = erpnext.taxes_and_totals.extend({
		...
		$.each(this.si_docs, function(index, data){
			for(key in data) {
				$(frappe.render_template("pos_bar_invoice_list", {
					...
					numero_mesa: data[key].numero_mesa,	//Bar e Restaurante
	...
	get_data_from_server: function (callback) {
		...
		frappe.call({
			...
			method: "angola_erp.bar_e_restaurante.doctype.pos.get_pos_data",
			freeze: true,
	...
	load_data: function (load_doc) {
		...
		if (load_doc) {
			...
			//Bar e Restaurante
			this.frm.atendbar = JSON.parse(localStorage.getItem('atendbar'));
	...
	make_search: function () {
		...
		this.mesas_campo = frappe.ui.form.make_control({
			df: {
				"fieldtype": "Data",
				"options": this.mesas,
				"label": this.mesas, //"Mesas",
				"fieldname": "mesa_cartao",
				"placeholder": __("Select Table or Card")
			},
			parent: this.wrapper.find(".search-mesa"),
			only_input: true,
		});
		this.mesas_campo.make_input();
		this.mesas_campo.$input.on("keyup", function() {
			setTimeout(function() {
				me.mesas = me.get_mesas();
				me.make_mesa_list();
			}, 1000);
		});
	...
	render_list_customers: function () {
		...
		if(this.si_docs.length) {
			this.si_docs.forEach(function (data, i) {
				for (key in data) {
					html += frappe.render_template("pos_bar_invoice_list", {
						sr: i + 1,
						name: key,
						customer: data[key].customer,
						numero_mesa: data[key].numero_mesa,	//Bar e Restaurante
		...
		if (this.items.length > 0) {
			$.each(this.items, function(index, obj) {
				if(index < me.page_len) {
					$(frappe.render_template("pos_bar_item", {
						item_code: obj.name,
	...
	bind_qty_event: function () {
		...
		$(this.wrapper).on("change", ".pos-item-qty", function () {
			var item_code = $(this).parents(".pos-bar-selected-item-action").attr("data-item-code");
			var qty = $(this).val();
	...
	render_selected_item: function() {
		...
		if(this.child_doc.length) {
			this.selected_row = $(frappe.render_template("pos_bar_selected_item", this.child_doc[0]))
	...
	update_qty_rate_against_item_code: function(item_code, field, value){
		...
		$.each(this.frm.atendbar["produtos"] || [], function(i, d) {
			if (d.produtos == item_code) {
				if (field=='qty') {
					d['quantidade'] = flt(value);
				}else if (field=='rate') {
					d['produtos_preco'] = flt(value);
				}
	...
	remove_zero_qty_item: function () {
		...
		this.frm.atendbar["produtos"] = this.produtos;	//Bar e Restaurante
	},
	...
	show_items_in_item_cart: function () {
		...
		$.each(this.frm.doc.items || [], function (i, d) {
			$(frappe.render_template("pos_bar_bill_item_new", {
	...
	change_status: function () {
		if (this.frm.doc.docstatus == 0) {
			...
			this.update_invoice();
			this.frm.atendbar.docstatus =1;	//Bar e Restaurante
	...
	create_invoice: function () {
		...
		var invoice_data = {}
		this.si_docs = this.get_doc_from_localstorage();
		var atendbar_data = {}					//Bar e Restaurante
		this.at_bar = this.get_atendbar_from_localstorage();	//Bar e Restaurante
		...
		$.each(this.si_docs, function (index, data) {
			for (key in data) {
				if (key == me.name) {
					...
					//Bar e Restaurante
					if (me.si_docs[index][key].docstatus == 1){
						//pay done so check if Cash or other
						for (c in me.si_docs[index][key].payments) {
							if (me.si_docs[index][key].payments[c].amount != 0){
								tipopagamento = me.si_docs[index][key].payments[c].mode_of_payment
	...
	update_localstorage: function () {
		try {
			localStorage.setItem('invoice_doc_bar', JSON.stringify(this.si_docs));
		} catch (e) {
			frappe.throw(__("LocalStorage is full , did not save"))
		}
	},
	...
	get_doc_from_localstorage: function () {
		try {
			return JSON.parse(localStorage.getItem('invoice_doc_bar')) || [];
		} catch (e) {
			return []
		}
	},
	...
	sync_sales_invoice: function () {
		...
		this.at_bar = this.get_submitted_atendimento();	//Bar e Restaurante
		...
		if (this.si_docs.length || this.email_queue_list || this.customers_list) {
			frappe.call({
				...
				method: "angola_erp.bar_e_restaurante.doctype.pos.make_invoice",
				args: {
					doc_list: me.si_docs,
					email_queue_list: me.email_queue_list,
					customers_list: me.customers_list,
					atendbar_list: me.at_bar
				},
				callback: function (r) {
					if (r.message) {
	...
	validate: function () {
		var me = this;
		this.customer_validate();
		this.mesas_validate();	//Bar e Restaurante
		this.item_validate();
	...
	//ATENDIMENTO BAR ROTINES

	get_submitted_atendimento: function(){
		var atendimento = [];
		var index = 1;
		docs = this.get_atendbar_from_localstorage();
		if(docs){
		...
		$.each(this.at_bar, function(index, data){
			for(key in data){
				if(key == me.name){
					...
					me.update_atendbar_localstorage();
				}
			}
		})
	},
	...
	get_atendbar_from_localstorage: function(){
		try{
			return JSON.parse(localStorage.getItem('bar_rest')) || [];
		}catch(e){
			return []
		}
	},

	remove_atendbar_from_localstorage: function(){
		var me = this;
		this.at_bar = this.get_atendbar_from_localstorage();
		this.new_at_bar = [];
	...
	update_atendbar_localstorage: function(){
		try{
			localStorage.setItem('bar_rest', JSON.stringify(this.at_bar));
		}catch(e){
			frappe.throw(__("LocalStorage is full , did not save"))
		}
	},

	make_mesa_list: function() {
		var me = this;
//		if(!this.price_list) {
//			msgprint(__("Price List not found or disabled"));
//			return;
//		}
		...
		var $wrap = me.wrapper.find(".mesa-list");
		me.wrapper.find(".mesa-list").empty();

		if (this.mesas.length > 0) {
			$.each(this.mesas, function(index, obj) {
				if(index < 30){
					$(frappe.render_template("pos_bar_mesas", {
						//nome_mesa: obj.name,
						numero_cartao: obj.numero_cartao,
						nome_mesa: obj.nome_mesa,
						mesa_image: obj.mesa_image ? "url('" + obj.mesa_image + "')" : null,
						status_mesa: obj.status_mesa,
						color: frappe.get_palette(obj.nome_mesa),
						abbr: frappe.get_abbr(obj.nome_mesa)
...
//		}
		...
		$(me.wrapper).find("div.pos-mesa").on("click", function() {
			me.customer_validate();
			if(me.frm.doc.docstatus==0) {
				//me.items = me.get_items($(this).attr("data-item-code"))
				//me.add_to_cart();
				...
				if ($(me.wrapper).find("div.pos-mesa-select").length !=0){
					...
					for (var x = 0; x < $(me.wrapper).find("div.pos-mesa-select").length; x++) {	
						//Marca como NAO selecionada
						var dd = $(me.wrapper).find("div.pos-mesa-select")[x]
						if ($(me.wrapper).find("div.pos-mesa-select")[x].className == "pos-mesa-select"){
	//							$("pos-mesa-select").appendTo($(me.wrapper).find("div.pos-mesa")[x].className)

							$(me.wrapper).find("div.pos-mesa-select")[x].className = "pos-mesa"
				...
				for (var x = 0; x < $(me.wrapper).find("div.pos-mesa").length; x++) {	
					if ($(me.wrapper).find("div.pos-mesa")[x].attributes[1].nodeValue  == mesaSelected) {
						//Marca como selecionada no Chrome
						if ($(me.wrapper).find("div.pos-mesa")[x].className == "pos-mesa"){

							$(me.wrapper).find("div.pos-mesa")[x].className = "pos-mesa-select"
						}

					}else if ($(me.wrapper).find("div.pos-mesa")[x].attributes[0].nodeValue  == mesaSelected) {
						//Marca como selecionada no IE
						if ($(me.wrapper).find("div.pos-mesa")[x].className == "pos-mesa"){
	...
	get_mesas: function(nome_mesa){
		...
		if(nome_mesa){
			return $.grep(this.mesas, function(item){
				if(item.nome_mesa == nome_mesa ){
					return true
				}
			})
		}

		key =  this.mesas_campo.$input.val().toLowerCase().replace(/[&\/\\#,+()\[\]$~.'":*?<>{}]/g,'\\$&');
		var re = new RegExp('%', 'g');
		var reg = new RegExp(key.replace(re, '[\\w*\\s*[a-zA-Z0-9]*]*'))
		search_status = true
...
//				alert("Ainda por ver")
				if(search_status){
					if(reg.test(item.nome_mesa.toLowerCase()) ){
						return true
					}else if (item.numero_cartao != null){
						if(reg.test(item.numero_cartao.toLowerCase()) ){
							return true
						}
					}
				}
