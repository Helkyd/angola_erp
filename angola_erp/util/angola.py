...
@frappe.whitelist()
def set_fees_pago(fatura):

	propinaspagar = frappe.model.frappe.get_all('propinas_pagar',filters={'parent':fatura},fields=['parent','propina'])
	for x in propinaspagar:
		pago = frappe.get_doc('Fees',x.propina)	
		print 'propina paga'
		print pago.outstanding_amount
		...
		if pago.outstanding_amount:
			#PAID
			if pago.grand_total:
				frappe.db.set_value("Fees", x.propina, "paid_amount", pago.grand_total)
			else:
				frappe.db.set_value("Fees", x.propina, "paid_amount", pago.total_amount)
			frappe.db.set_value("Fees", x.propina, "outstanding_amount", 0)
			#frappe.db.set_value("Fees", x.propina, "sales_invoice", fatura)
...
def get_programa_enroll(aluno):
	...
	comFee = frappe.db.sql(""" select p.name,p.student,p.student_name,p.program, f.parent,f.fee_structure,f.amount from `tabProgram Fee` f JOIN `tabProgram Enrollment` p on f.parent = p.name where p.student = %s; """, (aluno),as_dict=False)
	if comFee:
...
@frappe.whitelist()
def get_aluno_email_guardian(aluno):

	alunoemail = frappe.model.frappe.get_all('Student',filters={'name':aluno},fields=['name','student_email_id'])
	print alunoemail
	if alunoemail[0].student_email_id:
		return alunoemail
	else:	
		alunoguardian = frappe.model.frappe.get_all('Student Guardian',filters={'parent':aluno},fields=['name','parent','guardian'], order_by='guardian')
		print 'Aluno guardian'
		print alunoguardian	
		if alunoguardian:
			for x in alunoguardian:				
				alunoguardianemail = frappe.model.frappe.get_all('Guardian',filters={'name':x.guardian},fields=['name','email_address'])
				print alunoguardianemail
				if alunoguardianemail[0].email_address:
					#print alunoguardianemail
