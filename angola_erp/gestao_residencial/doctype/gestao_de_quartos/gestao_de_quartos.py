...
#					set_account_for_mode_of_payment(self)
	...
	def pagamentos_feitos(self):
		...
		#	pagopor = self.servico_pago_por


		temregisto = frappe.model.frappe.get_all('Sales Invoice Payment',filters={'parent':self.name, 'parenttype': 'Gestao de Quartos', 'type': self.pagamento_por},fields=['parent','parentfield','amount','base_amount', 'account'])
		...
		if not temregisto:
			...
			for x in range(1,2):
				...
				print 'type ', frappe.db.get_value("Mode of Payment", {"name": self.pagamento_por}, "type")
				print 'modo ', frappe.db.get_value("Mode of Payment", {"name": self.pagamento_por}, "mode_of_payment")
				pagamento_feito = frappe.get_doc({
					...
					"account": frappe.db.get_value("Mode of Payment Account", {"parent": self.pagamento_por, "company": self.company}, "default_account"),
					#self.return_against if cint(self.is_return) else self.name)
					"type": frappe.db.get_value("Mode of Payment", {"name": self.pagamento_por}, "type"),
					"mode_of_payment": frappe.db.get_value("Mode of Payment", {"name": self.pagamento_por}, "mode_of_payment"),
	#				"name": self.name,
					"idx": +x
				})
	...
	def pagamentos_feitos1(self):
		...
		if self.total_servicos:
			if 'Cash' in self.servico_pago_por:
			#if self.pagamento_por =='Cash':
				pagopor ="Cash"
			else:
			#elif self.servico_pago_por =='2-TPA':
			...
			print 'valor ', self.total_servicos
			print 'pago por ', pagopor
			print 'conta ', frappe.db.get_value("Mode of Payment Account", {"parent": "Cash" if (pagopor=="Cash") else pagopor, "company": self.company}, "default_account")
			print 'tipo ', frappe.db.get_value("Mode of Payment", {"name": pagopor}, "type")
			print 'modo ', frappe.db.get_value("Mode of Payment", {"name": pagopor}, "mode_of_payment")

#			frappe.db.sql("INSERT into `tabSales Invoice Payment` (name,parent, parentfield, parenttype, amount, base_amount, account, type, mode_of_payment) values (%s,'pagamento','Gestao de Quartos',%s,%s,%s,%s,%s) ",(self.name, self.total_servicos, self.total_servicos ,frappe.db.get_value("Mode of Payment Account", {"parent": "Cash" if (pagopor=="Cash") else "Bank", "company": self.company}, "default_account"), 'Cash', 'Cash'))

#			frappe.db.sql("INSERT into `tabSales Invoice Payment` (name,parent, parentfield, parenttype, amount, base_amount, account, type, mode_of_payment) values (%s,%s,'pagamento','Gestao de Quartos',%s,%s,%s,%s,%s) ",(make_autoname(self.name + '-' + '.#####'),self.name, self.total_servicos, self.total_servicos ,'45110000 - Caixa Kwanza - DP', 'Cash', 'Cash'))

#			frappe.db.commit()

# frappe.db.sql(" INSERT INTO `tabSales Invoice Payment` (parent, parentfield, parenttype, amount, base_amount, account, type, mode_of_payment) VALUES (%s,'pagamento','Gestao de Quartos',%s,%s,%s,%s,%s) ",(self.name, self.total_servicos,self.total_servicos,frappe.db.get_value("Mode of Payment Account", {"parent": "Cash" if (pagopor=="Cash") else "Bank", "company": self.company}, "default_account"),frappe.db.get_value("Mode of Payment", {"type": pagopor}, "type"),frappe.db.get_value("Mode of Payment", {"type": pagopor}, "mode_of_payment"))
...
#			frappe.db.sql("INSERT into tabMovimentos_Caixa (name, docstatus, parent, parenttype, parentfield, tipo_pagamento, descricao_movimento, valor_pago, hora_atendimento, creation, modified, usuario_movimentos, company) values (%s,0,%s,'Caixa de Registo','movimentos_caixa',%s,%s,%s,%s,%s,%s,%s,%s) ",(ddd, caixa, d.pagamento_por ,d.name, d.total_servicos, d.hora_atendimento, frappe.utils.now(), frappe.utils.now(),d.bar_tender,d.company))
			...
			pagamento_feito = frappe.get_doc({
				"doctype": "Sales Invoice Payment",
				"name": make_autoname(self.name + '-' + '.#####'),
				"parent": self.name,
				"parentfield": "pagamento",
				"parenttype": "Gestao de Quartos",
				"amount": self.total_servicos,
				"base_amount": self.total_servicos,
				"account": frappe.db.get_value("Mode of Payment Account", {"parent": "Cash" if (pagopor=="Cash") else pagopor, "company": self.company}, "default_account"),
				#self.return_against if cint(self.is_return) else self.name)
				"type": frappe.db.get_value("Mode of Payment", {"name": pagopor}, "type"),
				"mode_of_payment": frappe.db.get_value("Mode of Payment", {"name": pagopor}, "mode_of_payment")
				#"idx": 2
			})
			pagamento_feito.insert()
...
#			pagamento1_feito = frappe.new_doc('Sales Invoice Payment')
#			pagamento1_feito.parent = self.name
#			pagamento1_feito.parentfield = 'pagamento'
#			pagamento1_feito.parenttype = 'Gestao de Quartos'
#			pagamento1_feito.amount = self.total_servicos
#			pagamento1_feito.base_amount = self.total_servicos
#			pagamento1_feito.account = frappe.db.get_value("Mode of Payment Account", {"parent": "Cash" if (pagopor=="Cash") else "Bank", "company": self.company}, "default_account")

#			pagamento1_feito.type = frappe.db.get_value("Mode of Payment", {"type": pagopor}, "type")
#			pagamento1_feito.mode_of_payment = frappe.db.get_value("Mode of Payment", {"type": pagopor}, "mode_of_payment") 
#			pagamento_feito.idx = 2
#			pagamento1_feito.name = self.name
#			pagamento1_feito.insert()

#			frappe.db.sql("INSERT into `tabSales Invoice Payment` (name,parent, parentfield, parenttype, amount, base_amount, account, type, mode_of_payment) values (%s,%s,'pagamento','Gestao de Quartos',%s,%s,%s,%s,%s) ",(make_autoname(self.name + '-' + '.#####'),self.name, self.total_servicos, self.total_servicos ,'45110000 - Caixa Kwanza - DP', 'Cash', 'Cash'))



#			print(pagamento1_feito.idx)
#			print(pagamento1_feito.parent)
#			print(pagamento1_feito.amount)
...
#		pagamento_feito = frappe.new_doc("Sales Invoice Payment")
		...
		print 'nome ', self.name
		print 'conta ', frappe.db.get_value("Mode of Payment Account", {"parent": "Cash" if (pagopor=="Cash") else "Bank", "company": self.company}, "default_account")
		print 'tipo ', frappe.db.get_value("Mode of Payment", {"type": pagopor}, "type")
		print 'modo ', frappe.db.get_value("Mode of Payment", {"type": pagopor}, "mode_of_payment")
	...
	def make_pos_gl_entries(self, gl_entries):
			...
			for payment_mode in self.pagamento:
				if payment_mode.amount:
					...
					print 'make pos gl entries pagamentos...'
					print 'make pos gl entries pagamentos...'
					print 'make pos gl entries pagamentos...'
					gl_entries.append(
						self.get_gl_dict({
