...
//			cur_frm.doc.__unsaved = 1
		...
		console.log('pgamentos ', cur_frm.doc.pagamento)
		if (cur_frm.doc.__unsaved == undefined && cur_frm.doc.__islocal == undefined && cur_frm.doc.pagamento != undefined) {
			if (cur_frm.doc.pagamento.length == 0 && naorefresh == 0){
				naorefresh += 1
				cur_frm.refresh_fields("pagamento");	
				cur_frm.reload_doc()
				cur_frm.refresh_fields()
			}
		...
		if (modopagamento.responseJSON != undefined) {
...
//			if (cur_frm.doc.__unsaved == 1 && cur_frm.doc.__islocal == 1) {
...
//	if (modopagamento) {for (mod in modopagamento.responseJSON.message){(modopagamento.responseJSON.message[mod].parent) + "," }}
	...
	var d = frappe.prompt([
		...
		console.log(opcao0 + ' ' + opcao1 + ' ' + opcao2)
		console.log(opcao0_tipo + ' ' + opcao1_tipo + ' ' + opcao2_tipo)
