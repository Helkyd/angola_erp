...
#		for d in frappe.db.sql("""SELECT name,numero_quarto,hora_entrada,hora_saida,status_quarto FROM `tabGestao de Quartos` WHERE status_quarto = "Ocupado" or status_quarto = 'Ativo' and hora_saida <=%s """, (frappe.utils.now()), as_dict=True):
		versaida = frappe.db.sql("""SELECT name,numero_quarto,hora_entrada,hora_saida,status_quarto FROM `tabGestao de Quartos` WHERE status_quarto = "Ocupado" or status_quarto = 'Ativo' and hora_saida <=%s """, (frappe.utils.now()), as_dict=True)
		if versaida:
			for d in versaida:
	#			print "MINUTOS " + (frappe.utils.data.time_diff_in_seconds(frappe.utils.now(),d.hora_entrada)/60)
				if d.numero_quarto:
					...
					if (frappe.utils.data.time_diff_in_hours(frappe.utils.now(),d.hora_saida) > 0 and frappe.utils.data.time_diff_in_hours(frappe.utils.now(),d.hora_saida) <= 1): 
